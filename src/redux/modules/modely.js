...
// 가져오기
// Model Y Best Sellor
export const __loadBestSellor = (ModelYID1) => async(dispatch, getState) => {
    
    try {
        dispatch(loadBestSellerReq());
        const { data } = await apis.getModelYBest(ModelYID1)
        console.log(data);        
...
export const __loadInterior = (ModelYID2) => async(dispatch, getState) => {
    
    try {
        dispatch(loadInteriorReq());
        const { data } = await apis.getModelYInterior(ModelYID2)
        console.log(data);        
...
// Model Y Exterior
export const __loadExterior = (ModelYID3) => async(dispatch, getState) => {
    
    try {
        dispatch(loadExteriorReq());
        const { data } = await apis.getModelYExterior(ModelYID3)
        console.log(data);        
