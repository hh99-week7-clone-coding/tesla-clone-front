...
// 가져오기
// Model x Best Sellor
export const __loadBestSellor = (ModelXID1) => async(dispatch, getState) => {
    
    try {
        dispatch(loadBestSellerReq());
        const { data } = await apis.getModelXBest(ModelXID1)
        console.log(data);        
...
export const __loadInterior = (ModelXID2) => async(dispatch, getState) => {
    
    try {
        dispatch(loadInteriorReq());
        const { data } = await apis.getModelXInterior(ModelXID2)
        console.log(data);        
...
// Model x Exterior
export const __loadExterior = (ModelXID3) => async(dispatch, getState) => {
    
    try {
        dispatch(loadExteriorReq());
        const { data } = await apis.getModelXExterior(ModelXID3)
        console.log(data);        
